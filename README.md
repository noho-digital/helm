...
- `atomic`: if `false` will remove  `--atomic` from the args passed to helm to  prevent it from cleaning up and rolling back on failure. Only use when debugging, and not in production!   
